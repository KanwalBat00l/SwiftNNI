...
std::map<std::string, ModelProfile> ConfigManager::loadProfiles(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        while (std::getline(ss, item, ',')) row.push_back(trim(item));

        // Inside loadProfiles method, update the row parsing:
        if (row.size() >= 9) {
            ModelProfile p;
            p.model = row[0];
            ...
            // Initialize dynamic values with static baseline
