 ...
 * Energy(uJ) = (P_idle + p_J * P_dynamic_per_core) * TAT_seconds * 1e6
 * Where TAT_seconds = tat_ms / 1000.0
 */
long SystemMonitor::estimateEnergyLinearModel(int active_cores, long tat_ms,
                                               double p_idle,
                                               double p_dynamic_per_core) {
    double tat_sec = (double)tat_ms / 1000.0;
    double power_w = p_idle + (double)active_cores * p_dynamic_per_core;
    double energy_j = power_w * tat_sec;
    return (long)(energy_j * 1000000.0); // Convert joules to microjoules
}
